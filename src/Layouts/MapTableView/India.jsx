...
function India(props) {
  ...
  useEffect(() => {
    getDistrictData.then((distData) => {
      setdistrictData(distData);
    });
    getData.then((timeseriesData) => {
      ...
      let stateData = _.filter(InitialData, (e) => {
        return e.key !== "TT";
      });
      let stateTableData = _.filter(timeseriesData, (e) => {
        return e.key !== "TT";
      });
      let countryData = _.filter(timeseriesData, (e) => {
        return e.key === "TT";
      });
  ...
  }, [getData, getDistrictData]);
  function setSelectedState(id) {
    let selectedState = _.filter(data, (e) => {
      return e.key === id;
    });
    setSelected(selectedState[0]);
    let dist = _.filter(districtData, (e) => {
      return e.key === id;
    });
  ...
  function changeColor(color) {
    ...
    let stateData = _.filter(changedData, (e) => {
      return e.key !== "TT";
    });
    setMapData(stateData);
    setColor(color);
      ...
      <div className="md:grid grid-cols-2">
        <Fade left>
          <div>
            <Tooltip title={color}>
              <h1
                className={"text-2xl cursor-pointer uppercase font-bold  p-1 text-center bg-opacity-40  w-28 rounded-lg ".concat(
                  color === "confirmed"
                    ? "bg-tomato text-red-600"
                    : color === "active"
        ...
        <div className="block my-auto xl:mt-auto">
          {!loading ? (
            <Cards selected={selected} flex={false} grid={true} />
          ) : (
            ""
          )}
        </div>
        ...
        <div className="min-h-screen md:flex flex-row grid-cols-5 gap-2 mt-10">
          <Fade left>
            <div className="col-span-4 ">
              <p className="text-primary font-bold text-xl uppercase ">
                {" "}
                {selected.stateName}
              </p>
          ...
          <div className="mx-auto">
            <Cards
              selected={isDistrictSelected ? districtSelected : selected}
              flex={true}
              grid={false}
              myRef={myRef}
            />
